...
void ngap_uint32_to_GNB_ID(
    GNB_ID_PR present, c_uint32_t gnb_id, GNB_ID_t *gNB_ID)
{
    d_assert(gNB_ID, return, "Null param");

    gNB_ID->present = present;
    if (present == GNB_ID_PR_gNB_ID)
    {
...
void ngap_GNB_ID_to_uint32(GNB_ID_t *gNB_ID, c_uint32_t *uint32)
{
    d_assert(uint32, return, "Null param");
    d_assert(gNB_ID, return, "Null param");

    
    if (gNB_ID->present == GNB_ID_PR_gNB_ID)
    {
...
status_t ngap_BIT_STRING_to_ip(BIT_STRING_t *bit_string, ip_t *ip)
{
    ...
    d_assert(bit_string, return -1, "");
    d_assert(ip, return -1, "");
