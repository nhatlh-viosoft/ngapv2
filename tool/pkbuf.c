...
static clbuf_t* clbuf_alloc(c_uint16_t length)
{
    ...
    if (cluster == NULL)
    {
        ...
        printf("No more free cluster. length:%d requested", length);
        return NULL;
    }
...
static void clbuf_free(clbuf_t *clbuf)
{
    ...
    switch (clbuf->size)
    {
        ...
        case SIZEOF_CLUSTER_8192:
            ...
            break;
        default:
            printf("clbuf has invalid size %d", clbuf->size);
            return;
            break;
    }
...
pkbuf_t* pkbuf_alloc(c_uint16_t headroom, c_uint16_t length)
{
    ...
    if (headroom <= MAX_SIZEOF_HEADROOM)
    {
        printf("Max size of headroom is %d, but %d requested",
    ...
    if (clbuf == NULL)
    {
        printf("Can't allocate clbuf(length:%d)", length);
        return NULL;
    }
...
int pkbuf_header(pkbuf_t *pkbuf, c_int16_t increment)
{
    clbuf_t *clbuf;

    d_assert(pkbuf, return -1, "Null param");
    d_assert(pkbuf->clbuf, return -1, "pkbuf has no clbuf");
    d_assert(pkbuf->clbuf->cluster, return -1, "clbuf has no cluster");
...
void pkbuf_join(pkbuf_t *h, pkbuf_t *t)
{
    ...
    if (p->tot_len == p->len)
    {
        printf("p->tot_len(%d) == p->len(%d) (of last pbuf in chain)",
            p->tot_len, p->len);
        return ;
    }


    d_assert(p->next, return, "p->next == NULL");
...
int pkbuf_tobuf(pkbuf_t *pkbuf, void *buf, c_uint16_t *buflen)
{
    pkbuf_t *p;
    c_uint16_t copied = 0;

    d_assert(pkbuf, return -1, "Null param");
    d_assert(buf, return -1, "Null param");
    d_assert(buflen, return -1, "Null param");
    ...
    *buflen = 0;
    ...
    while (p)
    {
        d_assert(p->clbuf, return -1, "pkbuf has no clbuf");
        d_assert(p->clbuf->cluster, return -1, "clbuf has no cluster");
    ...
    d_assert(copied == pkbuf->tot_len, return -1,
...
int pkbuf_tobuf_partial(pkbuf_t *pkbuf, void *buf, c_uint16_t *buflen,
...
void *core_malloc(size_t size)
{
    ...
    if (p == NULL)
    {
        printf("pkbuf_alloc failed(headroom:%d, size:%d)", headroom, size);
        return NULL;
    }
    ...
    {
        pkbuf_free(p);
        printf("core_malloc should not be fragmented");
        return NULL;
    }
...
int core_free(void *ptr)
{
    ...
    d_assert(p, return -1, "Null param");
...
void *core_calloc(size_t nmemb, size_t size)
{
    ...
    if (ptr == NULL)
    {
        printf("nmeb = %d, sizeo = %d", nmemb, size);
        return NULL;
    }
...
char *core_strdup(const char *s)
{
    ...
    len = strlen(s) + 1;
    res = (char *)core_memdup((const void *)s, len);
    return res;
}
