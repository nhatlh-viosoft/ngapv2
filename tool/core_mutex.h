 ...
 */
CORE_DECLARE(int) mutex_init(void);

/**
 * Finalize Mutex Pool
 */
CORE_DECLARE(int) mutex_final(void);
 ...
 */
CORE_DECLARE(int) mutex_create(mutex_id *id, unsigned int flags);
/**
 * Acquire the lock for the given mutex. If the mutex is already locked,
 * the current thread will be put to sleep until the lock becomes available.
 * @param id the mutex on which to acquire the lock.
 */
CORE_DECLARE(int) mutex_lock(mutex_id id);
 ...
 */
CORE_DECLARE(int) mutex_trylock(mutex_id id);

/**
 * Release the lock for the given mutex.
 * @param id the mutex from which to release the lock.
 */
CORE_DECLARE(int) mutex_unlock(mutex_id id);

/**
 * Destroy the mutex and free the memory associated with the lock.
 * @param id the mutex to destroy.
 */
CORE_DECLARE(int) mutex_delete(mutex_id id);
